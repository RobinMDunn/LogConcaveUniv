...
results <- data.table::data.table(n_obs = n_obs, d = d, mu_norm = mu_norm,
                                  ...
                                  alpha = alpha, p_0 = p, reject = NA_real_)
...
for(row in 1:nrow(results)) {
  ...
  # Run partial oracle axis-aligned projection test to determine
  # whether to reject H_0
  test_out <- LogConcaveUniv::partial_oracle_axis(data = true_sample, B = B,
                                                  alpha = alpha)

  # Store whether to reject H_0
  results[row, reject := test_out$reject_null]

}

# Save simulation results
data.table::fwrite(results, file = paste0("sim_data/fig04_partial_oracle_axis_",
                                          line_number, ".csv"))
