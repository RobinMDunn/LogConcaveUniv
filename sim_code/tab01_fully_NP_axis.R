...
results <-
  data.table::data.table(Method = "Fully nonparametric, axis-aligned projections",
...
for(row in 1:nrow(results)) {
  ...
  # Run fully nonparametric axis-aligned projection test to determine
  # whether to reject H_0
  test_out <- LogConcaveUniv::fully_NP_axis(data = true_sample, B = B,
                                            alpha = alpha)
...
data.table::fwrite(results, file = paste0("sim_data/tab01_fully_NP_axis_",
                                          line_number, ".csv"))
