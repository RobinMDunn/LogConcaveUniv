...
# Read in data
perm_test <- fread("sim_data/fig05_perm_test.csv") %>% 
  mutate(Method = "Permutation test")

full_oracle_ddim <- fread("sim_data/fig05_full_oracle_ddim.csv") %>% 
  mutate(Method = "Full oracle, d-dim")

partial_oracle_ddim <- fread("sim_data/fig05_partial_oracle_ddim.csv") %>% 
  mutate(Method = "Partial oracle, d-dim")

partial_oracle_axis <- fread("sim_data/fig05_partial_oracle_axis.csv") %>% 
  mutate(Method = "Partial oracle, axis-aligned projections")

fully_NP_axis <- fread("sim_data/fig05_fully_NP_axis.csv") %>% 
  mutate(Method = "Fully nonparametric, axis-aligned projections")

partial_oracle_randproj <- fread("sim_data/fig05_partial_oracle_randproj.csv") %>% 
  mutate(Method = "Partial oracle, random projections")

fully_NP_randproj <- fread("sim_data/fig05_fully_NP_randproj.csv") %>% 
...
ggsave(plot = reject_props_equal_spaced,
       filename = "sim_plots/figure_05a.pdf",
       width = 13, height = 7)

ggsave(plot = reject_props_equal_zoom_onerow,
       filename = "sim_plots/figure_05b.pdf",
       width = 12, height = 3.3)
